...
export default function App() {
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        >
          ...
          >
            ...
            >
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQvZ9MGxOOTIIaSrTdZ7bwoS0W1Z5808q_LWg&s"></img>
            </motion.span>
            {/* Pulse ring */}
            <motion.div
  ...
  const renderPage4 = () => (
    <motion.div 
      ...
      style={{
        backgroundImage: `url('https://cache.careers360.mobi/media/colleges/social-media/media-gallery/17918/2019/3/18/Campus%20View%20of%20Government%20Polytechnic%20Chegunta_Campus-View.jpg')`,    "scripts": {
        backgroundSize: 'cover',
        backgroundPosition: 'center',
        backgroundRepeat: 'no-repeat'
      }}}')`,
    >   backgroundSize: 'cover',
      {/* Background overlay */}ter',
      <div className="absolute inset-0 bg-gradient-to-br from-green-50/90 via-white/95 to-emerald-50/90 z-0" />
      }}
      {/* Content wrapper */}
      <div className="relative z-10 flex flex-col min-h-screen">
        {/* Header */}absolute inset-0 bg-gradient-to-br from-green-50/90 via-white/95 to-emerald-50/90 z-0" />
        <motion.div 
          initial={{ y: -30, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }} flex-col min-h-screen">
          transition={{ delay: 0.1, duration: 0.5 }}
        ...
        > initial={{ y: -30, opacity: 0 }}
          ...
          <p className="text-green-600 mt-2 flex items-center justify-center gap-2">
            <CheckCircle className="w-5 h-5" />er-b border-green-100 bg-white/80 backdrop-blur-sm"
            Registration Complete
          </p>className="text-2xl md:text-3xl font-semibold text-gray-800">Government Polytechnic College Chegunta 519</h1>
        </motion.div>e="text-green-600 mt-2 flex items-center justify-center gap-2">
            <CheckCircle className="w-5 h-5" />
        {/* Final Details */}lete
        <div className="flex-1 p-6 flex items-center justify-center">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2, duration: 0.5 }}ustify-center">
            className="w-full max-w-2xl"
          > initial={{ opacity: 0, y: 30 }}
            <Card className="border-0 shadow-2xl bg-white/90 backdrop-blur-sm">
              <CardHeader className="text-center pb-8">
                <motion.divll max-w-2xl"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}adow-2xl bg-white/90 backdrop-blur-sm">
                  transition={{ delay: 0.3, duration: 0.6, type: "spring", stiffness: 200 }}
                >motion.div
                  <CardTitle className="text-center text-green-600">
                    <div className="mb-6">
                      <motion.div lay: 0.3, duration: 0.6, type: "spring", stiffness: 200 }}
                        className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6"
                        animate={{ rotate: [0, 10, -10, 0] }}n-600">
                        transition={{ delay: 0.5, duration: 0.6 }}
                      >motion.div 
                        <CheckCircle className="w-12 h-12 text-green-600" />items-center justify-center mx-auto mb-6"
                      </motion.div>rotate: [0, 10, -10, 0] }}
                      <motion.h2 n={{ delay: 0.5, duration: 0.6 }}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}-12 text-green-600" />
                        transition={{ delay: 0.6, duration: 0.5 }}
                        className="text-3xl font-bold"
                      > initial={{ opacity: 0, y: 10 }}
                        Registration Successful!: 0 }}
                      </motion.h2>={{ delay: 0.6, duration: 0.5 }}
                    </div>assName="text-3xl font-bold"
                  </CardTitle>
                </motion.div>tration Successful!
              </CardHeader>ion.h2>
              <CardContent className="space-y-6 p-8">
                <motion.div e>
                  initial={{ opacity: 0 }}
                ...
                </motion.div>{{ delay: 0.7, duration: 0.5 }}
                  className="text-center mb-8"
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}t-lg">Your registration has been completed successfully</p>
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.8, duration: 0.5 }}
                ...
                > initial={{ opacity: 0, y: 20 }}
                  {[imate={{ opacity: 1, y: 0 }}
                    { label: 'Name', value: formData.name, icon: User },
                    { label: 'Phone', value: formData.phone, icon: Phone },erald-50 p-8 rounded-2xl border border-green-200"
                    { label: 'Branch', value: formData.branch, icon: BookOpen },
                    { label: 'PIN', value: formData.pin, icon: MapPin },
                    { label: 'Caste', value: formData.caste, icon: Users },
                  ].map((item, index) => (e: formData.phone, icon: Phone },
                    <motion.div anch', value: formData.branch, icon: BookOpen },
                      key={item.label}lue: formData.pin, icon: MapPin },
                      initial={{ opacity: 0, x: -20 }}caste, icon: Users },
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 0.9 + index * 0.1, duration: 0.5 }}
                      className="flex items-center justify-between py-3"
                    > initial={{ opacity: 0, x: -20 }}
                      <span className="text-gray-600 flex items-center gap-3 text-lg">
                        <item.icon className="w-6 h-6 text-green-500" />0.5 }}
                        {item.label}: items-center justify-between py-3"
                      </span>
                      <span className="font-medium text-gray-800 text-lg">{item.value}</span>
                    </motion.div>n className="w-6 h-6 text-green-500" />
                  ))}   {item.label}:
                </motion.div>
                      <span className="font-medium text-gray-800 text-lg">{item.value}</span>
                <motion.div .div>
                  ...
                  className="space-y-4 pt-6"
                > initial={{ opacity: 0, y: 20 }}
                  <motion.divopacity: 1, y: 0 }}
                    whileHover={{ scale: 1.02 }}tion: 0.5 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <Button v
                      onClick={handlePrint}02 }}
                      className="w-full py-4 text-xl bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 shadow-lg hover:shadow-xl transition-all duration-300"
                    >
                      <span className="flex items-center justify-center gap-3">
                        <Printer className="w-6 h-6" />
                        Print Detailsll py-4 text-xl bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 shadow-lg hover:shadow-xl transition-all duration-300"
                      </span>
                    </Button>lassName="flex items-center justify-center gap-3">
                  </motion.div>r className="w-6 h-6" />
                        Print Details
                  <motion.div
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    ...
                    >   setFormData({ name: '', phone: '', branch: '', pin: '', caste: '' });
                      <span className="flex items-center justify-center gap-2">
                        ...
                        New Registrationpy-4 text-xl border-2 border-green-300 text-green-700 hover:bg-green-50 hover:border-green-400 transition-all duration-300"
                      </span>
                    </Button>lassName="flex items-center justify-center gap-2">
                  </motion.div>Ccw className="w-5 h-5" />
                </motion.div>egistration
              </CardContent>>
            </Card> </Button>
          </motion.div>ion.div>
        </div>  </motion.div>
        ...
        > initial={{ opacity: 0 }}
          <p className="text-gray-500">© GPTCC519</p>
        </motion.div>{{ delay: 1.5, duration: 0.5 }}
      </div>assName="text-center py-6"
    </motion.div>
  );      <p className="text-gray-500">© GPTCC519</p>
        </motion.div>
  // Render current page
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case 4: return renderPage4();
  return (ult: return renderPage1();
    <div className="w-full bg-white min-h-screen relative overflow-hidden">
      {/* Loading overlay */}
      <AnimatePresence>
        {isLoading && (
          <motion.div-full bg-white min-h-screen relative overflow-hidden">
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          > initial={{ opacity: 0 }}
            <motion.divopacity: 1 }}
              animate={{ rotate: 360 }}
              transition={{ duration: 1, repeat: Infinity, ease: "linear" }}ex items-center justify-center"
              className="w-8 h-8 border-3 border-blue-600 border-t-transparent rounded-full"
            />otion.div
          </motion.div>{ rotate: 360 }}
        )}    transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
      </AnimatePresence>"w-8 h-8 border-3 border-blue-600 border-t-transparent rounded-full"
            />
      {/* Page content with animations */}
      <AnimatePresence mode="wait">
        <motion.div key={currentPage}>
          {renderCurrentPage()}
        </motion.div>t with animations */}
      </AnimatePresence>ode="wait">
    </div>otion.div key={currentPage}>
  );      {renderCurrentPage()}
}       </motion.div>
      </AnimatePresence>
    </div>
